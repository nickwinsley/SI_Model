...
The recent COVID-19 pandemic highlighted the importance of data in informing policies and containing an epidemic. With the increasing importance of data in epidemiology, the need for contact tracing and data collection methods is ever-growing, and so too is the need for effective statistical methods to analyse contact tracing data. Recently, a 7-day trial of the CovidCard, a Bluetooth device for detecting personal contacts between carriers, was successfully commissioned by the New Zealand Ministry of Health. The CovidCard was shown to be an effective contact tracing method, performing better than traditional methods (@admiraal2022case).

Social Network Analysis (SNA) is used to study individuals and their relationship with each other. A wide array of SNA methods have been proposed for the analysis of contact tracing data, however many methods remain largely theoretical, and there is a lack of evidence-based consensus on which methods are most effective for the study of epidemiology. This study aims to develop a comprehensive methodology for identifying the most susceptible individuals during an epidemic, based on contact tracing data. A comparison of several existing SNA metrics, as well as a novel metric we propose (Absorption Rank), showed that predictions can be improved by considering many metrics. Additionally, common epidemic simulation assumptions were tested, demonstrating limitations in the external validity of popular simulation approaches.
...
mcr <- Multiplicative closeness rank\;
...
If some predictors in a model are unimportant, we risk losing model performance due to the "Curse of Dimensionality". Many unimportant predictors may result in increased over-fitting and decreased generalization to unseen data. Feature selection methods are used to identify and remove unimportant predictors in order to decrease model complexity and potentially increase model performance. The feature importance scores were calculated for a decision tree with all six predictors.
...
```{r, echo = FALSE}
library(rpart.plot)
rpart.plot(tree, main = "Visualization of the optimized decision tree")
```
...
The Hochberg procedure (@benjamini1995controlling) is a powerful method that performs well for positively correlated test statistics. It is well-suited for epidemiology data, where positive dependency between infection events is likely. The Hochberg Procedure generally provides greater power than Holm's method and the Bonferroni Correction.
...
```{r likeRatio, echo = FALSE, messages = FALSE}
