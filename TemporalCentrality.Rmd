...
where $I_{t,i,\textit{u}}$ is the influence domain of $\textit{u}$ over the time interval [t, i]. We will call this the temporal proximity prestige, or TP for short. The temporal proximity prestige can be normalized by dividing by i.
...
Figure 8 shows the Pearson correlations between the five metrics. Except for Proximity Rank and Closeness Rank, the metrics are mostly independent, with many weak or negative correlations. This suggests that the metrics are tapping into different effects, and our rankings could be improved by combining many metrics as predictors in a regression model.
...
holdoutMSE <- rep(NA, length(indices))
...
for (ind in indices){
  holdoutMSE[i] <- rmse(predict(all.models.fit.cv[[ind]], newdata = X[, predictors(all.models.fit.cv[[ind]])]), Y)^{2}
  i = i + 1
}

bestModels <- order(holdoutMSE)[1:5]
...
best.models <- cbind(round(as.numeric(unlist(holdoutMSE[bestModels[1:5]])), 6), best.models)

colnames(best.models) <- new_names

pander::pander(best.models, caption = "Five best-performing feature combinations on the holdout set and their MSE")
```

The best-performing model in the cross-validation phase used Absorption Rank, Temporal Katz Centrality and Degree Centrality. As expected, Absorption Centrality was included in all five models, however the inclusion of Temporal Proximity Prestige did not exclude Temporal Prestige. The standard deviation of cross-validation MSE scores was calculated for all feature combinations, and the MSE on the holdout set was calculated for all models within one standard deviation of the lowest model. Table 2 shows the five best-performing feature combinations on the holdout set. The best-performing model on the holdout set included Temporal Closeness Rank (TC), Katz Centrality and Degree Centrality as predictors, although these results should be interpreted with caution, as the holdout set comprised only 15% of the data set, therefore we would expect considerable sampling error (which potentially explains why the MSE estimates for the best-performing models on the holdout set are much lower than during cross-validation).

Each centrality metric was fitted in a simple linear regression model with one predictor, and the results were compared against the final decision tree model. Figure 12 compares the proportion of variance ($R^{2}$) in likelihood of infection explained by each centrality metric and the final decision tree chosen during cross-validation. 
...
full_rmse <- 1 - (min(all.models.fit.cv[[MSE.ordered[1]]]$results$RMSE)^{2}/sstY)
abs_rmse <- summary(lm(likelihood ~ Absorb, data = data))$r.squared
katz_rmse <- summary(lm(likelihood ~ Katz, data = data))$r.squared
deg_rmse <- summary(lm(likelihood ~ Degree, data = data))$r.squared
tp_rmse <- summary(lm(likelihood ~ TC, data = data))$r.squared
tpp_rmse <- summary(lm(likelihood ~ TP, data = data))$r.squared
tbc_rmse <- summary(lm(likelihood ~ TBC, data = data))$r.squared

rmse_vec <- c(full_rmse, abs_rmse, deg_rmse, katz_rmse, tp_rmse, tpp_rmse, tbc_rmse)

names(rmse_vec) <- c("Final Model", "Absorb", "Degree", "TP", "TBC", "TC", "Katz")
