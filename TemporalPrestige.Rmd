...
$$e^{\sum_{w \in E}{log(w)}}$$
To generalize conventional notions of centrality, we need to define distance in terms of probability. For this reason, I propose a new measure of distance, which I call the stochastic distance:

$$d^{\mathbf{p}} = 1 - \sum_{w \in E}{log(w)}$$
Note that the stohastic distance ranges from 1 to $\infty$, therefore we can adjust equations (4.) and (5.) to get
