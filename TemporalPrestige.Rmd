...
At present, the gold standard algorithm for calculating betweenness centrality, Brandes' algorithm (@brandes2001faster), has a runtime complexity of $V^{2} + VE$, where $V$ is the number of nodes and $E$ is the number of edges in the network. The novel algorithm we propose has a runtime complexity of $V(V - 1)(V - 2) \approx V^{3}$, which can be reduced to approximately $V^{2}$ if we calculate the centrality for a small number of nodes. Conversely, Brandes' algorithm requires $V^{2} + VE$ iterations regardless of the number of nodes we require centrality calculations for. Moreover, this novel approach is easily comprehensible and relies on simple data structures (e.g. matrix, 2D array, priority queue) which are either natively available in many programming languages, or easy to implement from built-in language features.
