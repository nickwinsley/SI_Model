...
<!--
```{r, echo = FALSE}
new_df <- data.frame(TP = log(data$TP), TPP = log(data$TPP), Absorb = log(data$Absorb), Katz = log(data$Katz), Degree = log(data$Degree), TBC = log(data$TBC), likelihood = data$likelihood)
```
...
<!--
```{r, echo = FALSE, fig.lab = "Pairwise relationships between predictor variables after locally linear embedding (LLE) transformation.", cache = TRUE}
 library(Rdimtools)
 
 X <- data[, -which(names(data) == "likelihood")]
 
 lle_result <- do.lle(as.matrix(X), ndim = 4, type = c("knn", 5))
 
 reduced_data <- data.frame(lle_result$Y)
 
 names(reduced_data) <- c("Component 1", "Component 2", "Component 3", "Component 4")
 ggpairs(reduced_data)
```

```{r, echo = FALSE}
data <- cbind(reduced_data, data$likelihood)
names(data) <- c(names(reduced_data), "likelihood")
```
-->
