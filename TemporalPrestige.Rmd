...
In some situations, the effect that removing a node has on transmission through a network may be highly important (for instance, when quarantining individuals during a pandemic). This is the primary motivation behind Betweenness Centrality (@freeman1977set), which is defined as
...
names <- c("Katz Centrality", "Degree Centrality", "Absorption Centrality", "Temporal Closeness", "Temporal Proximity")

df <- data.frame(type = names, corr = correlations)

df %>% ggplot(aes(x = names, y = corr)) + geom_bar(stat = "identity") + xlab("Centrality Measure") + ylab("Spearman Rank Correlation")
```
...
The Spearman Correlations are generally large, with Absorption Rank showing a larger correlation ($\rho \approx 0.7$) compared to other metrics. Conversely, all metrics showed relatively low Pearson Correlations. This suggests that the metrics should not be interpreted as a linearly increasing variable, rather they should be used for ranking the relative importance of individuals within the same network. 
...
Figure 8 shows the Pearson correlations between the five metrics. With the exception of Proximity Rank and Closeness Rank, the metrics are mostly independent. This suggests that our rankings could be improved by including many metrics as predictors in a regression model.
...
model = lm(probs ~ TP + TPP + Absorb + Katz + Degree, data = data)
...
qqline(residuals)
```
Figure 9 shows the Q-Q plot for a linear regression model predicting the observed likelihood of infection, with the five centrality metrics as predictors. The relationship is clearly non-linear, with a large number of extreme values for the residuals. 
