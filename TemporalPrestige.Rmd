...
## Novel Algorithm for Betweenness Centrality
...
\ForEach{node in nodes}{
  ...
  node <- Queue item being processed.\;
  ...
  \ForEach{node1, node2, node3 in nodes}{
    ...
    \If{dist[j, k] == dist[j, i] + dist[i, k]}{
      pbc[node1] = pbc[node1] + (numPaths[j, i] * numPaths[i, k])/numPaths[j, k]\;
    }
    \If{dist[i, k] == dist[i, j] + dist[j, k]}{
      pbc[node2] = pbc[node2] + (numPaths[i, j] * numPaths[j, k])/numPaths[i, k]\;
    }
    \If{dist[i, j] == dist[i, k] + dist[k, j]}{
      pbc[node3] = pbc[node3] + (numPaths[i, k] * numPaths[k, j])/numPaths[i, j]\;
    }
  }
  \Return(pbc/((n - 1) $\times$ (n - 2)))\;
}
\end{algorithm}

At present, the gold standard for calculating betweenness centrality, Brandes' algorithm (@brandes2001faster), has a runtime complexity of $V^{2} + VE$, where $V$ is the number of nodes and $E$ is the number of edges in the network. The novel algorithm we propose has a runtime complexity of $V(V - 1)(V - 2) \approx V^{3}$, which can be reduced to approximately $VE + V^{2}$ if we only calculate the centrality for a small number of nodes. Conversely, Brandes' algorithm requires $V^{2} + VE$ iterations regardless of the number of nodes we require centrality calculations for. Moreover, this novel approach is easily comprehensible and relies on simple data structures (e.g. matrix, 2D array, priority queue) which are either natively available in many programming languages, or easy to implement from built-in language features.
...
\begin{equation}
\label{eq:chi_square}
G = 2\sum_{ij}{n_{ij} ln \left(\frac{n_{ij}}{E_{ij}} \right)}
\end{equation}
