...
\ForEach{node in nodes}{
  ...
  \ForEach{node1, node2, node3 in nodes}{
    ...
    \If{dist[j, k] == dist[j, i] + dist[i, k]}{
      pbc[node1] = pbc[node1] + (numPaths[j, i] $\times$ numPaths[i, k])/numPaths[j, k]\;
    }
    \If{dist[i, k] == dist[i, j] + dist[j, k]}{
      pbc[node2] = pbc[node2] + (numPaths[i, j] $\times$ numPaths[j, k])/numPaths[i, k]\;
    }
    \If{dist[i, j] == dist[i, k] + dist[k, j]}{
      pbc[node3] = pbc[node3] + (numPaths[i, k] $\times$ numPaths[k, j])/numPaths[i, j]\;
    }
  }
